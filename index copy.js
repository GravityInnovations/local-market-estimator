...
function calculateOpportunity(population, comp, pricing) {
  // Weighted average session price across tier mix.
  // This is price per single visit — not a recurring weekly charge.
  const avgSessionPrice =
    pricing.min2 * 0.2 + pricing.min5 * 0.5 + pricing.min10 * 0.3;
  ...
  // How much of the online visibility gap exists for first-mover advantage
...
// Addressable pool (dynamic, not flat)
const basePenetration = 0.00008;

const competitionAdjustment =
  1 - clamp(comp.total / 25, 0, 0.5);

const addressableBase =
  population * basePenetration * (1 + competitionAdjustment);

// Capture rate (balanced + less optimistic bias)
const captureRate = clamp(
  ...
  const newCustomersMid = round(addressableBase * captureRate);
  const midMonthly = newCustomersMid * VISIT_FREQUENCY_MONTHLY * avgSessionPrice;

  return {
    avgSessionPrice,
    uplift: {
      low: breakdown(midMonthly * 0.7),
      mid: breakdown(midMonthly),
      high: breakdown(midMonthly * 1.3),
    },
    pressure: round(pressure, 3),
    visibilityGap: round(visibilityGap, 2),
    newCustomersMid,
    assumedVisitFrequencyMonthly: VISIT_FREQUENCY_MONTHLY,
  };
}
...
`;
  ...
  const systemPrompt = `You write concise, structured sales intelligence reports for tanning salon owners in the UK.
...
HARD OUTPUT RULES
========================
- Every array item MUST be ONE sentence only
- Maximum 16 words per sentence
- No explanations
- No reasoning words (no "because", "due to", "which leads to")
- No filler
- No duplication of ideas
- No extra formatting or punctuation beyond normal sentence

========================
MARKET SNAPSHOT (3 sentences)
========================
1. Population and tanning interest size
2. Competitor count and online discoverability level
3. Competitive pressure level impact
...
- Social media presence (or "unverified")
...
========================
Must include:
- online presence improvement (discoverability / digital footprint)
- social media improvement (Facebook, Instagram, TikTok)
- pricing transparency improvement (sessions or packages)
- local search visibility improvement

Each sentence must directly state action + business outcome.

========================
ESTIMATION (5 sentences)
========================
Each sentence must represent ONE independent metric derived ONLY from computed inputs.

Allowed metrics ONLY:

1. Monthly revenue uplift (£X–£Y range only, must come directly from uplift calculation)
2. Repeat visit frequency per customer (must be derived or explicitly computed in code)
3. New customer acquisition per month (must come directly from model output)
4. Conversion impact (ONLY allowed if explicitly calculated in code; otherwise output "not measurable from current model")
5. Visibility / brand strength (ONLY allowed if represented as a numeric score in input data; otherwise output "not measurable from current model")
Rules:
- Exactly 5 sentences total
- One sentence per metric
- Maximum 16 words per sentence
- No explanations, reasoning, or narrative framing
- No percentages unless explicitly provided in input data
- No invented deltas or derived assumptions outside code
- No qualitative statements (e.g. strong, weak, improved, better)
- Every numeric value must come from computed backend values only
- If a metric is not explicitly available, output:
  "Not measurable from current model"
- Repeat visit frequency must only change if explicitly recalculated in code
- Do not infer or estimate funnel metrics (conversion, visibility, engagement)

========================
GLOBAL LANGUAGE RULES
========================
- Never use the word "website"
- Use: "discoverable online", "online footprint", "digital presence", "appears in search results"
- No hedging words: "likely", "could", "might" are forbidden
